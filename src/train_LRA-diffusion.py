...
from utils.resnet import preact_resnet34
...
def test(diffusion_model, test_loader):
    ...
    with torch.no_grad():
        ...
        for test_batch_idx, data_batch in tqdm(enumerate(test_loader), total=len(test_loader), desc=f'Doing DDIM...', ncols=100):
            ...
            with torch.no_grad():
                label_t_0 = diffusion_model.reverse_ddim(images, stochastic=False).detach().cpu()
...
if __name__ == "__main__":
    ...
    diffusion_model = Diffusion(fp_encoder=fp_encoder, num_timesteps=1000, n_class=n_class, fp_dim=fp_dim,
                                feature_dim=2048, device=device)
